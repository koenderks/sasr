...
Bayesian inference involves first specifying a prior distribution that captures the available information about the probability of misstatement in the population. For illustrative purposes, we use a simple prior distribution that is indifferent about the possible values of the misstatement. Note that this prior distribution does not contain any information about the probability of misstatement, but yields statistical results that closely resemble classical outcomes.
...
After seeing a single observation, the prior is updated to the posterior via Bayes' theorem. Next, the posterior is used as a prior and is once again updated to a new posterior after seeing another observation. This can continue indefinately. After 10 observations, of which 0 contained a misstatement, the posterior distribution has the following shape.

```{r echo = FALSE}
eval <- evaluation(x = 0, n = 10, method = "binomial", prior = prior)
plot(eval)
```
