...
In many cases, you will download an R package from the [CRAN](http://cran.r-project.org/) server. This can be done via the `install.packages()` function by providing the package name in quotes. For example, an important package for data visualization is `ggplot2`. To install this package, you can simply run:
