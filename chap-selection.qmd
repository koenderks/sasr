...
```{r}
selection(
  data = BuildIt, size = planning(materiality = 0.03),
  units = "items", method = "random"
)
```
...
set.seed(1)
selection(
  data = BuildIt, size = n, order = "bookValue",
  units = "items", method = "random"
)$sample
```
...
set.seed(1)
selection(
  data = BuildIt, size = n, randomize = TRUE,
  units = "items", method = "random"
)$sample
```
...
selection(
  data = BuildIt, size = n,
  units = "items", method = "interval", start = start
)$sample
```
...
```{r}
selection(
  data = BuildIt, size = n, units = "values",
