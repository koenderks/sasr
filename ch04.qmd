...
the row indices of the items and store the result in a variable `rows`.
...
weigh the items and store the result in a variable `rows`.
...
To obtain which items are part of the sample, 

```{r}
items <- numeric(n)
for (i in 1:n) {
  item <- which(units[i] <= cumsum(BuildIt$bookValue))[1]
  items[i] <- (1:nrow(BuildIt))[item]
}
```
