...
yBreaks <- pretty(df$y, min.n = 4)
p <- ggplot(data = df, mapping = aes(x = x, y = y)) +
  geom_col(position = "identity", fill = c("firebrick", rep("darkgray", 10)), col = "black") +
  annotate(geom = "text", x = df$x, y = df$y, label = round(df$y, 3), vjust = -0.5) +
  scale_x_continuous(name = "Misstatements", breaks = 0:10) +
  scale_y_continuous(name = "Probability", breaks = yBreaks, limits = range(yBreaks)) +
...
yBreaks <- pretty(df$y, min.n = 4)
p <- ggplot(data = df, mapping = aes(x = x, y = y)) +
  geom_col(position = "identity", fill = c(rep("firebrick", 2), rep("darkgray", 9)), col = "black") +
  annotate(geom = "text", x = df$x, y = df$y, label = round(df$y, 3), vjust = -0.5) +
  scale_x_continuous(name = "Misstatements", breaks = 0:10) +
  scale_y_continuous(name = "Probability", breaks = yBreaks, limits = range(yBreaks)) +
  geom_segment(x = -Inf, xend = -Inf, y = min(yBreaks), yend = max(yBreaks)) +
...
p <- ggplot(data = benchmark, mapping = aes(x = sales, y = costofsales)) +
  ...
  geom_point(data = data.frame(x = 298112312, y = 223994405), shape = 21,
             mapping = aes(x = x, y = y), fill = "dodgerblue",
             inherit.aes = FALSE, show.legend = FALSE) +
  scale_x_continuous(name = "Sales (Million $)", breaks = breaks, limits = range(breaks)) +
  scale_y_continuous(name = "Cost of sales (Million $)", breaks = breaks, limits = range(breaks)) +
...
breaks <- pretty(c(benchmark$sales, benchmark$costofsales))
p <- ggplot(data = benchmark, mapping = aes(x = sales, y = costofsales)) +
  geom_smooth(stat = "smooth", formula = y ~ x, method = "lm", col = "black") +
  geom_point(shape = 21, fill = "darkgray") +
  geom_point(data = data.frame(x = 298112312, y = 223994405), shape = 21,
             mapping = aes(x = x, y = y), fill = "dodgerblue",
             inherit.aes = FALSE, show.legend = FALSE) +
  geom_point(data = data.frame(x = 298112312, y = C_hat), shape = 21,
             mapping = aes(x = x, y = y), fill = "firebrick",
             inherit.aes = FALSE, show.legend = FALSE) +
  scale_x_continuous(name = "Sales (Million $)", breaks = breaks, limits = range(breaks)) +
  scale_y_continuous(name = "Cost of sales (Million $)", breaks = breaks, limits = range(breaks)) +
  geom_segment(x = -Inf, xend = -Inf, y = min(breaks), yend = max(breaks)) +
...
The prior distribution can be constructed through a call to `auditPrior()`:
