...
items and $K$ assumed misstatements in the population.
...
binomial distrib ution, auditing standards round the tolerable misstatements
upwards to a whole number [@AICPA-A]. For example, if we try to use want to use
`expected = 1.5`, **jfa** will use `2` as the expected misstatements. The
resulting sample size is $n = 208$.
...
$n$ items given an assumed misstatement probability $\theta_{max}$.
...
in the population equal to, or higher than, 3%.
