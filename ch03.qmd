...
![Figure 3.1: The standard audit sampling workflow consists of four stages: planning, selection, execution, and evaluation. The planning stage is the first of these stages. ](img/workflow_planning.png){fig-align="center"}
...
![Figure 3.2. Properly planning your sample size can be key to ensuring an efficient audit. Image available under a [CC-BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/legalcode) license.](img/book_planning.png){fig-align="center" width=75%}
...
The auditor can incorporate information obtained through predictive analytics
into the prior distribution for $\theta$. In this example, we place ourselves in
the shoes of an auditor that is auditing a client they have audited for ten
years. The population under consideration is:
...
Because the auditor is auditing a long-term client, the auditor has access to
all samples of the last ten years. These samples are gathered in a single data
set called `history`.
...
The goal of the analytical procedure is to make a prediction about the number of
possible misstatements in the population. Hence, the auditor fits a predictive
model to predict the variable `k` in the training data `history`.

```{r}
fit <- randomForest::randomForest(formula = k ~ x1 + x2, data = history)
```

The predictions for the unknown `k` in the `population` data can be obtained by
...
This distribution does not have a shape that resembles any known distribution.
