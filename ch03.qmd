...
![When the sampling objectives, the expected misstatements and the likelihood are set, there exists one sample size that minimizes the amount of audit effort. Image available under a [CC-BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/legalcode) license.](img/book_puzzle.png){fig-align="center" width=75%}
...
In classical planning using the Poisson likelihood, the following statistical model 
is specified:
...
information about the probability of misstatement obtained through predictive
analytics into the prior distribution for $\theta$. In this example, the auditor
is conducting an audit on a long-term client that they have been working with
for ten years. Hence, the auditor has access to a historycal data set called
`history`, which contains the samples from all of the previous audits that have
been conducted on this auditee over the past ten years.

```{r}
set.seed(1)
history <- data.frame(k = factor(c(rep(0, 3300), rep(1, 200))),
                      ftes = runif(3500, 0, 26),
                      days = runif(3500, 1, 10))
head(history)
```
...
colnames(population)[4:5] <- c("ftes", "days")
head(population)
```
...
as Random Forest [@iemand] to learn the relationship between misstatements, the
number of full-time equivalent employees, and the number of outstanding days in
...
probabilities that each item in the population is accurately stated. In contrast
to the previous example, this distribution is not a parametric distribution,
which means we are unable to utilize any parametric priors from **jfa**.
However, by providing samples of the prior distribution, **jfa** is able to
construct a prior internally.
...
The performance materiality for this example is set to $\theta_{max}$ = 2%, and
the audit risk is set to $\alpha = 0.05$, or 5%. The minimum sample size can be
calculated with the command below and is 99.

```{r}
plan <- planning(materiality = 0.02, likelihood = "binomial", prior = prior)
summary(plan)
```
...
By using a frequentist approach, the required minimum sample size is 149. Thus,
by performing the analytical procedure and incorporating this information into
the prior distribution, the auditor has achieved a reduction in sample size of
50 items.
