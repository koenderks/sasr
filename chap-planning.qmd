...
We can make this sample size visually intuitive by showing the hypergeometric($k$ | 94, 1000, 30) distribution and highlighting the probability for $k = 0$, see Figure \ref{fig:planning-hyper-nomis}. This probability is lower than the required sampling risk $\alpha = 0.05$.

```{r fig.cap="\\label{fig:planning-hyper-nomis}The hypergeometric($k$ | 94, 1000, 30) distribution showing the probability for $k = 0$ misstatements as a red bar."}
plot(plan)
```
...
Like before, we can make this sample size visually intuitive by showing the hypergeometric($k$ | 147, 1000, 30) distribution and highlighting the probabilities for $k = 0$ and $k = 1$, see Figure \ref{fig:planning-hyper-mis}. The sum of these probabilities is lower than the required sampling risk $\alpha = 0.05$.

```{r fig.cap="\\label{fig:planning-hyper-mis}The hypergeometric($k$ | 147, 1000, 30) distribution showing the probability for $k = 0$ and $k = 1$ misstatements as a red bar."}
plot(plan)
```
...
Like before, you can inspect how the prior distribution compares to the expected posterior distribution by using the `plot()` function, see Figure \ref{fig:planning-bbinom-mis} below for the output of this call.

```{r fig.cap="\\label{fig:planning-bbinom-mis}The beta-binomial prior and posterior distribution on the range [$k$; $N - n + k$] after seeing one misstatement in the sample."}
plot(plan)
```
...
We can make this sample size visually intuitive by showing the binomial($k$ | 99, 0.03) distribution and highlighting the probability for $k = 0$, see Figure \ref{fig:planning-binom-nomis}. This probability is lower than the required sampling risk $\alpha = 0.05$.

```{r fig.cap="\\label{fig:planning-binom-nomis}The binomial($k$ | 99, 0.03) distribution showing the probability for $k = 0$ misstatement as a red bar."}
plot(plan)
```
...
Like before, we can make this sample size visually intuitive by showing the binomial($k$ | 157, 0.03) distribution and highlighting the probabilities for $k = 0$ and $k = 1$, see Figure \ref{fig:planning-binom-mis}. The sum of these probabilities is lower than the required sampling risk $\alpha = 0.05$.

```{r fig.cap="\\label{fig:planning-binom-mis}The binomial($k$ | 157, 0.03) distribution showing the probability for $k = 0$ and $k=1$ misstatements as a red bar."}
plot(plan)
```
...
You can inspect how the prior distribution compares to the expected posterior distribution by using the `plot()` function, see Figure \ref{fig:planning-beta-nomis}. The expected posterior distribution is the posterior distribution that would occur if you actually observed the planned sample containing the expected misstatements.

```{r fig.cap="\\label{fig:planning-beta-nomis}The beta prior and posterior distribution on the range [0; 1] after seeing no misstatements in the sample of 98 units."}
plot(plan)
```
...
The resulting sample size under this prior is $n = 90$, a reduction of 8 samples when compared to the default beta(1, 1) prior distribution. Figure \ref{fig:planning-beta-nomis2} shows this prior and posterior distribution.

```{r fig.cap="\\label{fig:planning-beta-nomis2}The beta prior distributions in this example contains risk-reducing information. The posterior distribution has roughly the same upper bound as the one in the previous exxample and occurs after seeing no misstatements in the sample of 90 units."}
plot(plan)
```
...
We can make this visually intuitive by showing the Poisson($k$ | 100 $\times$ 0.03) distribution and highlighting the probability for $k = 0$, see Figure \ref{fig:planning-pois-nomis}. This probability is lower than the required sampling risk $\alpha = 0.05$.

```{r fig.cap="\\label{fig:planning-pois-nomis}The Poisson($k$ | 100 $\times$ 0.03) distribution showing the probability for $k = 0$ misstatement as a red bar."}
plot(plan)
```
...
Like before, we can make this visually intuitive by showing the Poisson($k$ | 159 $\times$ 0.03) distribution and highlighting the probabilities for $k = 0$ and $k = 1$, see Figure \ref{fig:planning-pois-mis}. The sum of these probabilities is lower than the required sampling risk $\alpha = 0.05$.

```{r fig.cap="\\label{fig:planning-pois-mis}The Poisson($k$ | 159 $\times$ 0.03) distribution showing the probability for $k = 0$ and $k=1$ misstatements as a red bar."}
plot(plan)
```
...
You can inspect how the prior distribution compares to the expected posterior distribution by using the `plot()` function, see Figure \ref{planning-gamma-nomis}. The expected posterior distribution is the posterior distribution that would occur if you actually observed the planned sample containing the expected misstatements.

```{r fig.cap="\\label{fig:planning-gamma-nomis}The gamma prior and posterior distribution on the range [0; $\\infty$] after seeing no misstatements in the sample of 99 units."}
plot(plan)
```
...
The resulting sample size under this prior is $n = 91$, a reduction of 8 samples when compared to the default gamma(1, 1) prior. Figure \ref{fig:planning-gamma-nomis2} shows this prior and posterior distribution.

```{r fig.cap="\\label{fig:planning-gamma-nomis2}The gamma prior distributions in this example contains risk-reducing information. The posterior distribution has roughly the same upper bound as the one in the previous exxample and occurs after seeing no misstatements in the sample of 91 units."}
plot(plan)
```
...
All prior distributions can be visually inspected via the `plot()` function, see Figure \ref{fig:planning-prior-default}.

```{r fig.cap="\\label{fig:planning-prior-default}The default beta(1, 1) prior distribution."}
plot(prior)
```
...
The predictions of the prior distribution can be visualized using the `plot()` function, see Figure \ref{fig:planning-prior-predict}.

```{r fig.cap="\\label{fig:planning-prior-predict}The predictions of the beta(1, 1) prior distribution concerming the possible misstatements in an an intended sample of $n=6$."}
...
You can incorporate the assumption that tolerable misstatement is equally likely as intolerable misstatement [@Derks2022b] using `method = 'impartial'`. For example, the command below creates an impartial beta prior distribution for a performance materiality of 5 percent. This method requires that you specify a value for the `materiality`.
...
Furthermore, the prior distribution can be visualized with a call to the `plot()` function, see Figure \ref{fig:planning-prior-example1}.

```{r fig.cap="\\label{fig:planning-prior-example1}The prior distribution constructed on the basis of the assessments of inherent risk and control risk."}
plot(prior)
```
...
The auditee's the sum of the sales is $298,112,312 and the sum of the booked costs of sales is $223,994,405, respectively. This is indicated by a blue dot in Figure \ref{fig:planning-scatter-1} below, which visualizes the industry sales versus the cost of sales.

```{r}
C_real <- 223994405
```

```{r fig.cap="\\label{fig:planning-scatter-1}Scatter plot of the industry sales versus the cost of sales.", echo = FALSE, fig.width = 7, fig.height = 6, fig.align = "center"}
...
The fitted regression line and the predicted cost of sales (red dot) are visualized in Figure \ref{fig:planning-scatter-2} below.

```{r fig.cap="\\label{fig:planning-scatter-2}Scatter plot of the industry sales versus the cost of sales including the regression line and the auditee's (predicted) cost of sales.", echo = FALSE, fig.width = 7, fig.height = 6, fig.align = "center"}
breaks <- pretty(c(benchmark$sales, benchmark$costofsales))
labels <- paste0("$", format(breaks / 1000000, scientific = FALSE), "M")
p <- ggplot(data = benchmark, mapping = aes(x = sales, y = costofsales)) +
  geom_smooth(
...
The specified prior distribution can be visualized using the `plot()` function, see Figure \ref{fig:planning-prior-example2}.

```{r fig.cap="\\label{fig:planning-prior-example2}The prior distribution constructed on the basis of the benchmark analysis."}
plot(prior)
```
...
You can inspect how the prior distribution compares to the expected posterior distribution by using the `plot()` function, see Figure \ref{fig:planning-prior-posterior-example2}.

```{r fig.cap="\\label{fig:planning-prior-posterior-example2}The prior and expected posterior distribution for this example after seeing a sample of $n=50$ containing no misstatements."}
plot(plan)
```
...
The nonparametric prior distribution can be visualized using the `plot()` function, see Figure \ref{fig:planning-prior-example3}.

```{r fig.cap="\\label{fig:planning-prior-example3}The prior distribution constructed on the basis of the predictive model."}
plot(prior)
```
...
You can inspect how the prior distribution compares to the expected posterior distribution by using the `plot()` function, see \ref{fig:planning-prior-posterior-example3}.

```{r fig.cap="\\label{fig:planning-prior-posterior-example3}The prior and expected posterior distribution for this example after seeing a sample of $n=108$ units containing no misstatements."}
plot(plan)
```
...
1. Use the classical approach with the hypergeometric likelihood to compute the minimum required sample size for a population of $N = 100$ when applying a performance materiality of 3 percent and a sampling risk of 5 percent. Tolerate no misstatements in the sample.
...
3. Use the classical approach with the binomial likelihood to compute the minimum required sample size for a performance materiality of 4.4 percent and a sampling risk of 5 percent. Tolerate one misstatement in the sample.
...
5. Use the classical approach with the Poisson likelihood to compute the minimum required sample size for a performance materiality of 2 percent and a sampling risk of 5 percent. Use an expected misstatement rate of 0.5 percent.
