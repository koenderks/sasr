...
Like before, you can inspect how the prior distribution compares to the expected posterior distribution by using the `plot()` function, see Figure \ref{fig:planning-bbinom-mis}.

```{r fig.cap="\\label{fig:planning-bbinom-mis}The beta-binomial prior and posterior distribution on the range [$k$; $N - n + k$] after seeing misstatements in the sample."}
plot(plan)
```
